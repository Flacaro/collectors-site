...
export class EditDiskComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.editDiskForm = this.formBuilder.group({
      ...
      this.disk$ = this.diskService.getPersonalDiskById(this.collectionId, this.diskId);
  ...
  onSubmit() {
    this.diskService.editPersonalDisk(this.collectionId, this.diskId, this.editDiskForm.value).subscribe();
